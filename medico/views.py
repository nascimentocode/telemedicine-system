...
def abrir_horario(request):
    if not is_medico(request.user):
        messages.add_message(
            request, constants.WARNING, "Somente médicos podem abrir horários"
        )
...
def consultas_medico(request):
    ...
    consulta = Consulta.objects.filter(data_aberta__user=request.user).filter(
        data_aberta__data__lt=hoje
    )
    ...
    ).exclude(id__in=consultas_hoje.values("id"))

    # if especialidade_consulta_filtrar:
    #     consultas_hoje = consultas_hoje.filter()
    #     consultas_restantes = consultas_restantes.filter()
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        if consulta.status == "C":
            messages.add_message(
                request, constants.WARNING, "Essa consulta já foi cancelada"
            )
            return redirect(f"/medicos/consulta_area_medico/{id_consulta}")
        elif consulta.status == "F":
            messages.add_message(
                request, constants.WARNING, "Essa consulta já foi finalizada"
            )
            return redirect(f"/medicos/consulta_area_medico/{id_consulta}")
